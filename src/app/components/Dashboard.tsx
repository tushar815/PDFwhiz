...
const Dashboard = () => {
    ...
    const utils = trpc.useContext() // forces refresh
    const {data: files, isLoading} = trpc.getUserFiles.useQuery()

    const {mutate: deleteFile} = trpc.deleteFile.useMutation({
        onSuccess: () => {
            utils.getUserFiles.invalidate();
        }, // on getting success from api
        onMutate: ({id}) => {
            setCurrentlyDeletingFile(id);
        },    //called right away after clicking on button,
        onSettled: () => {
                    ...
                    {files.sort(
                        ...
                        .map((file) => (
                            <li className='col-span-1 divide-y divide-gray-200 rounded-lg bg-white
                             shadow transition hover:shadow-lg' key={file.id}>
                                <Link className='flex flex-col gap-2' href={`/dashboard/${file.id}`}>
                                    <div className='pt-6 px-6 flex w-full items-center justify-between space-x-6'>
                                        <div
                                            className='h-10 w-10 flex-shrink-0 rounded-full bg-gradient-to-r from-rose-300 to to-red-500'/>
